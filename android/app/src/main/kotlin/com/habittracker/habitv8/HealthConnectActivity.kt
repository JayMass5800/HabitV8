...
// MindfulnessSessionRecord not available in alpha10, will be added when stable
...
class HealthConnectActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            try {
                // Skip feature check for alpha versions to avoid experimental API warnings
                Log.i(TAG, "Health Connect client initialized successfully")
                Log.i(TAG, "Proceeding with permission check...")
    ...
    private fun getHealthPermissions(): Set<String> {
        ...
        try {
            ...
            // Note: MindfulnessSessionRecord not available in Health Connect alpha10
            // Will be re-enabled when stable version is available
            Log.i(TAG, "Skipping MindfulnessSessionRecord (not available in current Health Connect version)")
        } catch (e: Exception) {
