...
class AutomaticHabitCompletionService {
  ...
  static const String _completionIntervalKey = 'completion_check_interval_minutes';
  ...
  static Stream<AutoCompletionEvent>? get eventStream => _eventController?.stream;
  ...
  static Future<bool> isServiceEnabled() async {
    try {
      ...
      return prefs.getBool(_serviceEnabledKey) ?? true; // Default enabled
    } catch (e) {
    ...
    try {
      ...
      return {
        ...
        'lastCheckDate': lastCheck > 0 ? 
            DateTime.fromMillisecondsSinceEpoch(lastCheck).toIso8601String() : null,
        'checkIntervalMinutes': intervalMinutes,
        'nextCheckTime': _isRunning && lastCheck > 0 ? 
            lastCheck + (intervalMinutes * 60 * 1000) : null,
      };
    } catch (e) {
  ...
  static Future<List<Map<String, dynamic>>> getCompletionHistory({int limit = 50}) async {
    try {
      final prefs = await SharedPreferences.getInstance();
  ...
  static Future<AutoCompletionResult> _performCompletionCheck() async {
    ...
    try {
      ...
      final prefs = await SharedPreferences.getInstance();
      await prefs.setInt(_lastCompletionCheckKey, DateTime.now().millisecondsSinceEpoch);
      ...
      for (final habit in habits) {
        try {
          ...
          AppLogger.error('Error checking habit ${habit.name} for completion', e);
      ...
      _eventController?.add(AutoCompletionEvent(
        ...
        message: 'Completion check finished: ${result.completedHabits}/${result.checkedHabits} habits completed',
        timestamp: now,
      ));
      
      AppLogger.info('Completion check finished: ${result.completedHabits}/${result.checkedHabits} habits auto-completed');
  ...
  static Future<void> _addToCompletionHistory(HabitAutoCompletion completion) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final historyJson = prefs.getStringList(_completionHistoryKey) ?? [];
  ...
  static Future<void> _sendCompletionNotification(List<HabitAutoCompletion> completions) async {
    try {
      ...
      if (completions.length == 1) {
        ...
        body = '${completion.habitName} was automatically marked as complete based on your health data.';
      } else {
        title = 'Multiple Habits Auto-Completed! 🎉';
        body = '${completions.length} habits were automatically completed based on your health data.';
      }
  ...
  static String _serializeCompletionHistoryEntry(HabitAutoCompletion completion) {
