...
class AIService {
  ...
  Future<List<Map<String, dynamic>>> generateOpenAIInsights(List<Habit> habits) async {
      ...
      final response = await http.post(
        ...
        body: jsonEncode({
          ...
          'messages': [
            {
              'role': 'system',
              'content': 'You are a habit coach AI assistant. Analyze habit data and provide personalized insights and recommendations. Return exactly 3 insights in JSON format with fields: type, title, description, icon.'
            },
            {
              'role': 'user',
              'content': 'Analyze these habits and provide insights: $habitSummary'
            }
          ],
          'max_tokens': 1000,
          'temperature': 0.7,
  ...
  Future<List<Map<String, dynamic>>> generateGeminiInsights(List<Habit> habits) async {
          ...
          'contents': [
            {
              'parts': [
                {
                  'text': '''Analyze these habit tracking data and provide exactly 3 personalized insights.
...
[
  ...
  String _generateHabitSummary(List<Habit> habits) {
    ...
    final bestStreak = streaks.isNotEmpty ? streaks.reduce((a, b) => a > b ? a : b) : 0;
...
''';
  ...
  String _getRecentTrends(List<Habit> habits) {
    ...
    for (final habit in habits) {
      recentCompletions += habit.completions.where((c) => 
          c.isAfter(lastWeek) && c.isBefore(now.add(const Duration(days: 1)))).length;
      previousCompletions += habit.completions.where((c) => 
          c.isAfter(previousWeek) && c.isBefore(lastWeek.add(const Duration(days: 1)))).length;
    }
    ...
    return [
      {
        'type': 'motivational',
        'title': 'Keep Building',
        'description': 'Every small step counts towards building lasting habits. Your consistency matters more than perfection.',
        'icon': 'rocket_launch',
      },
      {
        'type': 'pattern',
        'title': 'Track Your Progress',
        'description': 'Continue logging your habits to unlock personalized AI insights and recommendations.',
        'icon': 'trending_up',
      },
      {
        'type': 'achievement',
        'title': 'You\'re Growing',
        'description': 'Each day you track is a step towards better self-awareness and habit mastery.',
        'icon': 'emoji_events',
      },
    ];
  }
  ...
  List<String> get availableProviders {
    ...
    if (_openAiApiKey != null && _openAiApiKey!.isNotEmpty) providers.add('OpenAI');
    if (_geminiApiKey != null && _geminiApiKey!.isNotEmpty) providers.add('Gemini');
    return providers;
  }
}
