...
class WorkManagerHabitService {
  ...
  static const String _bootCompletionTaskName = 'com.habitv8.BOOT_COMPLETION_TASK';
    ...
    try {
      ...
      await Workmanager().registerPeriodicTask(
        ...
        constraints: Constraints(
          ...
          batteryNotLow: false, // Allow execution even when battery is low
      ...
      AppLogger.info('🔄 Scheduled periodic renewal task (interval: ${intervalHours}h)');
    } catch (e) {
  ...
  static Future<void> _handleBootCompletion() async {
    try {
      AppLogger.info('🔄 Handling boot completion - rescheduling all active habits');
    ...
    try {
      ...
      if (lastRenewalStr != null) {
        try {
          ...
          AppLogger.warning('Invalid last renewal date format: ');
        }
      }
      ...
      if (lastRenewal == null) {
        ...
        if (hoursSinceRenewal >= 6) {
          needsRenewal = true;
          AppLogger.info(' hours since last renewal, renewal needed');
        } else {
          AppLogger.info(' hours since last renewal, no renewal needed yet');
        }
      }
      ...
      if (needsRenewal) {
        ...
        AppLogger.info('✅ Habit continuation renewal completed and timestamp updated');
      }
    } catch (e) {
  ...
  static Future<void> _performHabitContinuationRenewal({
    ...
    try {
      AppLogger.info('🔄 Starting habit continuation renewal process${forceRenewal ? ' (forced)' : ''}${specificHabitId != null ? ' for habit ID: ' : ''}');
      ...
      if (specificHabitId != null) {
        ...
        habitsToRenew = habits.where((h) => h.id == specificHabitId && h.isActive).toList();
      } else {
        // Otherwise, renew all active habits
        habitsToRenew = habits.where((h) => h.isActive).toList();
      }
      
      AppLogger.info('🔄 Renewing continuation for  active habits');
        ...
        try {
          ...
          if (shouldRenew) {
            if (habit.notificationsEnabled) {
              ...
              renewedCount++;
              AppLogger.info('✅ Renewed notifications for habit: ');
            }
          } else {
            AppLogger.info('⏭️ Skipped renewal for habit:  (not scheduled for current time/day)');
          }
        } catch (e) {
          errorCount++;
          AppLogger.error('❌ Error renewing habit: ', e);
        }
      }
      
      AppLogger.info('✅ Habit continuation renewal completed: $renewedCount renewed, $errorCount errors');
    } catch (e) {
  ...
  static bool _shouldRenewHabit(dynamic habit, {bool forceRenewal = false}) {
    ...
    switch (habit.frequency) {
      ...
      case HabitFrequency.yearly:
        ...
        if (habit.selectedYearlyDates != null && habit.selectedYearlyDates.isNotEmpty) {
  ...
  Future<void> _renewHabitNotifications(dynamic habit) async {
    try {
      ...
      switch (habit.frequency) {
        case HabitFrequency.hourly:
          await NotificationService.scheduleHourlyHabitNotifications(habit);
          break;
        case HabitFrequency.daily:
          await NotificationService.scheduleDailyHabitNotifications(habit);
          break;
        case HabitFrequency.weekly:
          await NotificationService.scheduleWeeklyHabitNotifications(habit);
          break;
        case HabitFrequency.monthly:
          await NotificationService.scheduleMonthlyHabitNotifications(habit);
          break;
        case HabitFrequency.yearly:
          await NotificationService.scheduleYearlyHabitNotifications(habit);
          break;
      }
    } catch (e) {
      AppLogger.error('❌ Error renewing notifications for habit: ', e);
      rethrow;
    }
  }
  ...
  Future<void> forceRenewal({String? specificHabitId}) async {
    AppLogger.info('🔄 Force renewal requested${specificHabitId != null ? ' for habit ID: ' : ''}');
  ...
  Future<void> setRenewalInterval(int hours) async {
    if (hours < 1 || hours > 24) {
    ...
    AppLogger.info('🔄 Renewal interval set to  hours');
  }
